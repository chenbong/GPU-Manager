...
from .slimmable_ops import USConv2d, USLinear
...
class Block(nn.Module):
    def __init__(self, cin, outp, stride, tmp_ratio=1.0):
        ...
        expand_ratio = 0.25
        layers = [
            USConv2d(cin, midp, 1, 1, 0, bias=False, ratio=[tmp_ratio, expand_ratio]),
        ...
        self.residual_connection = stride == 1 and cin == outp
        if not self.residual_connection:
            self.shortcut = nn.Sequential(
                USConv2d(cin, outp, 1, stride=stride, bias=False, ratio=[tmp_ratio, 1]),
                USBatchNorm2d(outp),
            )
...
class Model(nn.Module):
    def __init__(self, n_classes=1000, input_size=224):
        ...
        self.classifier = nn.Sequential(
            USLinear(self.outp, n_classes, us=[True, False])
        )
